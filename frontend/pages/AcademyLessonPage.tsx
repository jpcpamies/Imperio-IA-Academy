...
export function AcademyLessonPage() {
  ...
  const handleMarkComplete = async () => {
    ...
    try {
      // For now, we'll use a mock user ID since we don't have user progress integration yet
      await backend.progress.markComplete({
        userId: 1, // This should be user.id when we integrate user progress
        lessonId: lesson.id,
      });
      setCompleted(true);
      toast({
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <Link 
          to={`/academy/courses/${lesson.courseId}`} 
